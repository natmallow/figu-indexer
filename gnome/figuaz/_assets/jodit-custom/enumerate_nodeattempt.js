...
enumerate = () => {
  return {
    ...
    exec: (editor) => {
        ...
        const regP =
          /.*?(?<! rd| v| d| Tab| vs| dr| engl| nb| d\.h| s| pp| p| Erkl| z\.z| bzw| dr| u\.a| Anm| \(Anm| e\.g| Mr| Mrs| Ms| resp| i\.e| z\.B| u\.U| o\.Ä| ca| Chr| usw| usf| evtl| e\.V| Jahrh| lat| m\.E| n\.V| z\.T| etc| jr| jun| a\.m| p\.m)[.!?\u2026](?=(?<!\d\.|\d\s\.)\s+|<|\s*$)/gi;
...
// given 2 string references and a count
function enumerator(countRef, stringRef1, stringRef2, ref1, ref2) {
  // console.log(stringRef1);
  //hard coded
  const r1Postfx = 'en';
  const r2Postfx = 'de';
  const prefx = 'track';
  ...
  // matches the string with the pattern regex
  const ref1Matches = stringRef1.match(regP);
  const ref2Matches = stringRef2.match(regP);

  // solution for cutoffs ref1
  if (ref1Matches && ref1Matches.length) {
    const ref1MatLen = ref1Matches.join('').length ?? 0;
    let ref1LeftOver = stringRef1.slice(ref1MatLen);
    if (ref1LeftOver.trim().length != 0) {
      ref1Matches.push(ref1LeftOver);
    }
  }
  
  // solution for cutoffs ref2        
  if (ref2Matches && ref2Matches.length) {
    const ref2MatLen = ref2Matches.join('').length ?? 0;
    let ref2LeftOver = stringRef2.slice(ref2MatLen);
    if (ref2LeftOver.trim().length != 0) {
      ref2Matches.push(ref2LeftOver);
    }
  }

  // rebuild the nodeValue here
  try {
    longestLen =
    ref1Matches.length > ref2Matches.length
        ? ref1Matches.length
        : ref2Matches.length;
    while (i < longestLen) {
      const cntR1 = `${prefx}_${countRef}_${i}-${r1Postfx}`;      
      const cntR2 = `${prefx}_${countRef}_${i}-${r2Postfx}`;

      if (ref1Matches[i] && ref2Matches[i]) {
        ref1htm += `<track-span id="${cntR1}" class="eNum">${ref1Matches[i]}</track-span>`;
        ref2htm += `<track-span id="${cntR2}" class="eNum">${ref2Matches[i]}</track-span>`;
      } else if (!ref1Matches[i] && ref2Matches[i]) {
        ref1htm += `<track-span id="${cntR1}" class="eNum missing"> - missing </track-span>`;
        ref2htm += `<track-span id="${cntR2}" style="color:red;" class="eNum missing">${ref2Matches[i]}</track-span>`;
      } else if (ref1Matches[i] && !ref2Matches[i]) {
        ref1htm += `<track-span id="${cntR1}" style="color:red;" class="eNum missing">${ref1Matches[i]}</track-span>`;
        ref2htm += `<track-span id="${cntR2}" class="eNum missing"> - missing </track-span>`;
      }
  ...
  // if there are no regP matches
  try {
    const cntR1 = `${prefx}_${countRef}_${i}-${r1Postfx}`;      
    const cntR2 = `${prefx}_${countRef}_${i}-${r2Postfx}`;
    // if there were no matches just retun org html
    if (!ref1Matches || !ref2Matches) {
      ref1htm = `<track-span id="${cntR1}" class="eNum" >${stringRef1.nodeValue}</track-span>`;
      ref2htm = `<track-span id="${cntR2}" class="eNum" >${stringRef2.nodeValue}</track-span>`;
    }
  } catch (err) {}

  ref1.innerHTML  = ref1htm;
  ref2.innerHTML  = ref2htm;
      ...
      function nested_table($gerTable, $engTable, $curcount) {
       ...
       $gerNested.forEach((element, ind) => {
        ...
        $gerTd.forEach((ele, ind) => {
            ...
            // enum all segments 
            const $gerMatches = $gerStr.match(regP);
            const $engMatches = $engStr.match(regP);
            ...
            $nestCount++;
            // restoreBr()
            $gerTd[ind].innerHTML = $gerhtm; 
            $engTd[ind].innerHTML = $enghtm;
...
// right here should be the nodeHandler 


// clean Html htmlEncode htmlDecode
          const col1 = $html.querySelector(":scope > td:first-child");
          const col2 = $html.querySelector(":scope > td:last-child");

          const col1Blocks = getTextNodesIn(col1)
          ...
          const col2Blocks = getTextNodesIn(col2, function(textNode, parent) {
            if (/^\s+$/.test(textNode.nodeValue)) {
              parent.removeChild(textNode);
            } else {
              return textNode;
...
//           const mLen = col1Blocks.length >= col2Blocks.length? col1Blocks.length : col2Blocks.length; 
// for (n=0;n<mLen;n++) {}
  // const col1n = col1Blocks.hasOwnProperty(n)? col1Blocks[n]:'';
  // const col2n = col2Blocks.hasOwnProperty(n)? col2Blocks[n]:'';
  const col1n = col1Blocks.map(u => u.nodeValue).join(' ');
  const col2n = col2Blocks.map(u => u.nodeValue).join(' ');
// console.log(col1Blocks)
  enumerator(
          $curcount,
          col1n,
          col2n,
...
// getTextNodesIn(mainTable)

// getTextNodesIn(mainTable, function(textNode, parent) {
//   if (/^\s+$/.test(textNode.nodeValue)) {
//     parent.removeChild(textNode);
//   } else {
...
// });
   //  console.log(getTextNodesIn(mainTable))

      for (let i = 0; i < nodeList.length; i++) {
          // htmlRtn.appendChild(format_FOMK(nodeList[i], i));
          
          format_FOMK(nodeList[i], i);
      }

      // editor.value = htmlRtn.outerHTML + metaDataEl;
      editor.value += metaDataEl;
      // editor.value = mainTable.outerHTML + metaDataEl;
