  ...
  return {
    ...
    exec: (editor) => {
      ...
      function format_FOMK($html, $curcount = 1, $isTable = false) {
        ...
        const $re =
          /.*?(?<! ca| Chr| u\.U| o\.Ä| usw| usf| z\.B| NB| e\.g| bzw| z\.Z| evtl| resp| Anm| d\.h| e\.V| geb| Jahrh| lat| m\.E| n\.V| u\.a| z\.T| i\.e| etc| jr| jun| a\.m| p\.m)[.!?\u2026](?=(?<!\d\.|\d\s\.)\s+|\s*$)/gi;
        ...
        $html.querySelector(":scope > td:first-child").innerHTML = $enghtm;
        $html.querySelector(":scope > td:last-child").innerHTML = $gerhtm;
      ...
      // set scrolling elements
      scrollElements(editor.container.querySelectorAll(".missing"));
    },
  };
};
...
removeEnumerate = () => {
  return {
    ...
    exec: (editor) => {
     ...
     // set scrolling elements
      scrollElements(editor.container.querySelectorAll(".missing"), true);  
...
highlightAbbreviationsPublication = () => {
  ...
  return {
    ...
    exec: (editor) => {     
      ...
      scrollElements(editor.container.querySelectorAll(".decision"));
    }
  }
}

countModule = (function () {
  let state = 0; // Private Variable

  let pub = {}; // public object - returned at end of module
...
arrayModule = (function () {
  ...
  return pub; // expose externally
        ...
        // eleArr = [];
// loc = 0;
scrollElements = (eleArr=null, remove=false) => {
    
        if (eleArr !== null && remove) {
  console.log('remove true', eleArr)
          countModule.changeState(0);
          // let difference = arrayModule.getState().filter(x => eleArr.indexOf(x) === -1);
          let b = new Set(eleArr);
          let difference = [...arrayModule.getState()].filter(x => !b.has(x));
          arrayModule.changeState(difference);
          return;
        } else if (eleArr !== null && !remove) {
          console.log('remove false', eleArr)
          countModule.changeState(0);
          let joined = [...new Set([...arrayModule.getState(),...eleArr])]
          arrayModule.changeState(joined);
          return;
        }

        var loc = 1*countModule.getState();
        var elements = arrayModule.getState(); 
console.log(loc,'---1')
        if(!elements.length){
          return;
        }         

        if (loc >= arrayModule.getState().length) {
          countModule.changeState(0);
        } else {
          const nLoc = loc+1;
          countModule.changeState(nLoc);          
        }
console.log(loc,'---2')
        if (eleArr === null) {
          return {'elements':elements,'loc':loc};
        }
...
nextDiscrepancy = () => {
  return {
    ...
    exec: (editor) => {
        ...
        const se = scrollElements();
        if (!se.elements) return
        se.elements[se.loc].scrollIntoView({behavior: "smooth", block: "center", inline: "nearest"});

        // countModule.changeState(++loc);
    ...
    //     return;
    //   }
    //  editor.container.querySelectorAll(".missing")
    //   .scrollIntoView({behavior: "smooth", block: "center", inline: "nearest"});
