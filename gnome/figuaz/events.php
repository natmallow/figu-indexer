...
$s = $database->dbc->query($query);
$total_results = $s->fetchColumn();
$total_pages = floor($total_results/$limit);

$startLimit = (int)($page-1)*$limit;

$sql = "SELECT event_id, event_date, name 
        FROM event 
        ...
        <div id="wrapper">
            <div id="main">
                <div class="inner">
                    ...
                    <section>
                        <header class="main">
                            <h2 style="display: inline-block;">You are editing Events <small>(in)</small> <span style="color:darkorange;"><?php echo ($lang == 'en') ? 'English' : 'Spanish' ?></span></h2>

                        </header>

                        <div class="row gtr-200">
                            <div class="col-4 col-5-medium">
                                Events - <a href="/gnome/event.php?action=add&lang=<?= $lang ?>" class="button small" style="line-height: 4em;">Create New Event</a>
                                <hr>
                            </div>
...
//style="display: flex; flex-wrap: wrap; justify-content: space-between"
                            ...
                            <div class="row alt-rows" style="">
                                <div class="col-10"  >
                                    <?= is_null($events[$i]["name"]) ? 'needed' : $events[$i]["name"]; ?>
                                </div>
                                <div class="col-2" style="text-align: right; padding-right: 5px;" >
                                    <a href="/gnome/event.php?id=<?= $events[$i]["event_id"] ?>&action=edit&lang=<?= $lang ?>" style="padding-top: .4em;">Edit</a> 
                                </div>
                            </div>
                            <?php
                        endfor;
